...
class BookmarkTreeStorage {
  private _bookmarks: IBookmarkTreeStorage[] = [];
   ...
   * 현재 북마크 트리 반환
   */
  public getBookmarks(): IBookmarkTreeStorage[] {
   ...
   * 북마크 트리를 직접 설정
   */
  public setBookmarks(bookmarks: IBookmarkTreeStorage[]): void {
    this._bookmarks = bookmarks;
  }

  public findNodeById(nodeId: string): IBookmarkTreeStorage | null {
    const searchInNodes = (
      nodes: IBookmarkTreeStorage[],
    ): IBookmarkTreeStorage | null => {
  ...
  public findParentNode(targetNodeId: string): IBookmarkTreeStorage | null {
    const searchInNodes = (
      nodes: IBookmarkTreeStorage[],
    ): IBookmarkTreeStorage | null => {
      for (const node of nodes) {
  ...
  private _areAllChildrenSelected(node: IBookmarkTreeStorage): boolean {
  ...
  private _hasAnySelectedChildren(node: IBookmarkTreeStorage): boolean {
    if (!node.children || node.children.length === 0) {
      return false;
    }
  ...
  public clearAllSelections(): void {
    const clearNode = (node: IBookmarkTreeStorage): void => {
   ...
   * 선택된 모든 노드들을 반환
   */
  public getSelectedNodes(): IBookmarkTreeStorage[] {
  ...
  public selectAll(): void {
    const selectNode = (node: IBookmarkTreeStorage): void => {
...
export const bookmarkTreeStorage: BookmarkTreeStorage =
  new BookmarkTreeStorage();
